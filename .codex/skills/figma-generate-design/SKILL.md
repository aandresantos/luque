...
4. **Identify the product's font family from the source. Do not default to Inter.** Find _which_ typeface the product uses before writing any script. See [references/discover-product-font.md](references/discover-product-font.md) for where to look (CSS variables, component files) and how to resolve messy Figma font names.
