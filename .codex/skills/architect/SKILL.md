...
back-end/src/modules/<entity>/
  routes.ts       → Fastify plugin (export default fp(plugin))
  handler.ts      → Thin handlers, delegate to service
  service.ts      → Business logic, calls repository
  repository.ts   → ONLY file allowed to query Drizzle
...
- **Files to create/modify**:
  - `schema.ts` — (describe table or changes)
  - `dto.ts` — (list Zod schemas: CreateXDto, UpdateXDto, XResponseDto)
  - `repository.ts` — (list methods: findById, findAll, create, update, delete)
  - `service.ts` — (describe business logic per use case)
  - `handler.ts` — (list handler functions and what they do)
  - `routes.ts` — (list: METHOD /path → handlerName)
